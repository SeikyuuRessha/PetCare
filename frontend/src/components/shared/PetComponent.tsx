...
export default function PetComponent({ pet, onViewDetails, hideViewDetails = false, className = '' }: PetComponentProps) {
  return (
    <div className={`bg-[#f3f8f7] border-2 border-[#8bc34a] rounded-xl p-6 flex flex-col min-w-[380px] max-w-[420px] ${className}`}>
      <div className="flex items-center">
        <div>
          <img
            src={pet.imageUrl || "/pet-avatar.png"}
            alt="Pet"
        ...
        <div className="ml-8 flex flex-col gap-1 text-base">
          <div>
            ...
            <span className="font-bold text-black ml-1">{pet.name}</span>
          </div>
          <div>
            <span className="text-[#b2c800] font-semibold">Chủ Nhân:</span>
            <span className="text-black ml-1">{pet.owner}</span>
          </div>
          <div>
